...
execute("perl $RealBin/1.make.input.pl --hosts $args{hosts} --pathogens $args{pathogens} --blast-json json/blast.hits.json --group Test --query-list $args{'query-list-file'} --query-species $args{'query-species'}");
execute("perl $RealBin/2.find.regions.pl --hosts $args{hosts} --pathogens $args{pathogens} --query-species \"$args{'query-species'}\"");
